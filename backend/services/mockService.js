/**
 * Serviço de Mock Inteligente (Fallback)
 * Simula a classificação, análise e recomendação financeira de IA
 * usando regras heurísticas baseadas em palavras-chave em português.
 */
...
/**
 * Determina a categoria, tipo e gera análises personalizadas com base na transação
 * @param {Object} transaction - Objeto { descricao, valor, data }
 * @returns {Object} - Resposta mockada no mesmo formato JSON esperado da OpenAI
 */
export function generateMockAIResponse(transaction) {
  ...
  // 1. Identificar Receitas (Salário, Pix recebido, etc)
  ...
  if (ehReceita) {
    ...
    // 2. Classificação de Despesas baseada em Heurísticas
  ...
  // 3. Geração de Análise, Recomendação e Alerta Personalizados
  const valorFormatado = new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(valor);

  if (tipo === 'receita') {
    analise = `Excelente! O recebimento de ${valorFormatado} referente a "${descricao}" aumenta sua liquidez e fortalece seu saldo mensal.`;
    ...
    switch (categoria) {
      ...
      case CATEGORIAS.TRANSPORTE:
        ...
        if (descNormalizada.includes('uber') || descNormalizada.includes('99')) {
          ...
          alerta = valor > 80;
        } else if (descNormalizada.includes('gasolina') || descNormalizada.includes('combustivel')) {
          ...
          recomendacao = 'Planeje suas rotas com antecedência para agrupar compromissos em uma mesma viagem e economizar tempo e combustível.';
        }
        break;
...
/**
 * Fallback inteligente para o Chatbot Financeiro
 * Gera respostas contextualizadas baseadas nas finanças do usuário no momento
 * @param {string} question - Dúvida do usuário
 * @param {Object} financialContext - Resumo contendo { saldo, totalReceitas, totalDespesas, maiorCategoriaGasto, quantidadeTransacoes, metas }
 * @returns {string} - Resposta simulada da IA
 */
export function generateMockChatResponse(question, financialContext) {
  ...
  // 1. Caso não existam dados suficientes
  ...
  // 2. Situação Financeira Geral (Pedido no TESTE 4)
  if (questNormalizada.includes('situacao') || questNormalizada.includes('financeira') || questNormalizada.includes('resumo') || questNormalizada.includes('meu estado')) {
    let situacao = `Com base nos seus dados atuais, aqui está o resumo consolidado da sua **situação financeira**:\n\n`;
  ...
  // 3. Análise de onde está gastando mais (Pedido no TESTE 4)
  ...
  // 5. Análise de Economia / Como economizar (Pedido no TESTE 4)
  ...
  // 6. Análise de Gastos Altos / Despesas
  ...
  // 9. Resposta Genérica Educativa
  return "Olá! Sou o seu Consultor Financeiro FinanAI. \n\nComo assistente educativo, posso te ajudar a analisar seu orçamento atual de forma muito simples. Tente me perguntar coisas como:\n- *'Me explique meu saldo atual'* \n- *'Como posso economizar?'* \n- *'Quais são minhas maiores despesas?'* \n\nMinha missão é guiar você para a independência financeira através de hábitos saudáveis, organização de metas e investimentos constantes!";
}
